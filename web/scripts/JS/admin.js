...
let handleDeleteSongFromPlaylist = (obj, songId, playlistId) => {
    ...
    formData.append("action", "deleteSongFromPlaylist");
        ...
        body: formData
            ...
            .then(result => {
                if (result.status === 200) {
                    ...
                    handleRemoveRow(obj); // Remove the song row from the table
                } else {
    ...
    if (playlistId !== "null") {
        ...
        if (response !== null || response.ok) {
            ...
            for (var s of songObj) {
                ...
                imgCell.classList.add("song-img");
                imgCell.innerHTML = `<img src="${s.songImagePath}" alt="User picture here" />`;
                songHTML.appendChild(imgCell);

                let titleCell = document.createElement("td");
                titleCell.classList.add("song-title");
                titleCell.textContent = s.title;
                songHTML.appendChild(titleCell);

                let artistCell = document.createElement("td");
                artistCell.classList.add("song-artist");
                artistCell.textContent = s.artist;
                songHTML.appendChild(artistCell);

                let albumCell = document.createElement("td");
                albumCell.classList.add("song-album");
                albumCell.textContent = s.album;
                songHTML.appendChild(albumCell);

                let durationCell = document.createElement("td");
                durationCell.classList.add("song-duration");
                durationCell.textContent = s.duration;
                songHTML.appendChild(durationCell);

                let actionCell = document.createElement("td");
                let delBtn = document.createElement("button");
                delBtn.classList.add("edit-playlist__delete-btn");
                delBtn.innerHTML = `<i class="fa-solid fa-trash-can"></i>`;
                actionCell.appendChild(delBtn);
                songHTML.appendChild(actionCell);

                // Add the event listener dynamically because I'm dumb
                delBtn.addEventListener("click", () => {
                    console.log("Song id in button: " + s.songId);
